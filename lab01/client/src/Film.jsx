...
function mapToFilm(e){
    return new Film(e.id,e.title,e.favorite,e.watchdate ? dayjs(e.watchdate) : null,e.rating);
}

function mapFromFilm(e){
    return {'id' : e.id , 'title' : e.title , 'favorite' : e.favorite ? e.favorite : false , 'watchdate' : e.date ? dayjs(e.date).format("YYYY-MM-DD") : null , 'rating' : e.rating , 'user' : e.user?e.user:1}
}

    
export default Film
export {mapToFilm , mapFromFilm}